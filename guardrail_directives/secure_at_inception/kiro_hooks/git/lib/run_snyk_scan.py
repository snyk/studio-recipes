...
class DependencyVulnerability:
    ...
    def __repr__(self) -> str:
        return (
            f"{self.severity.upper()}: {self.title} in {self.package_name}@{self.installed_version}"
        )
...
def run_sast_scan(target_path: str = ".") -> SastScanResult:
    ...
    return SastScanResult(success=True, vulnerabilities=vulnerabilities)
...
def run_sca_scan(target_path: str = ".") -> ScaScanResult:
    ...
    return ScaScanResult(success=True, vulnerabilities=vulnerabilities)
...
def run_sca_scan_for_package(package_name: str, version: str) -> ScaScanResult:
    ...
    if exit_code < 0 or exit_code > 1:
        return ScaScanResult(
            success=False, error_message=stderr or f"Failed to scan {package_name}@{version}"
        )
