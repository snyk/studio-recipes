...
_AUTH_ERROR_PATTERNS: Tuple[str, ...] = (
    ...
    # opaque CLI error and counts toward disabling scanning for the session.
    "snyk auth",
)
...
def resolve_log_file(workspace: str) -> str:
    ...
    Unix path: ``~/.snyk-studio/ades/claude/ws/<workspace-name>/log.txt``.
    Windows path ``C:\\Users\\<user>\\.snyk-studio\\ades\\claude\\ws\\<name>\\log.txt``.
    """
    name = _safe_workspace_name(workspace)
    return os.path.join(
        os.path.expanduser("~"), ".snyk-studio", "ades", "claude", "ws", name, "log.txt"
    )
...
def log(message: str, log_file: str, *, debug: bool = False) -> None:
    ...
    entries (debug=True) are written only when CLAUDE_HOOK_DEBUG=1. The
    ...
    if not log_file:
        return
    if debug and os.environ.get("CLAUDE_HOOK_DEBUG") != "1":
        return
    try:
        # Set the restrictive perms (dir 0700, file 0600) at creation time via
        # umask rather than chmod-ing the path afterwards: this avoids passing
        # the externally-supplied log path to a privileged path-sink (os.chmod)
        # and is also free of the create-then-chmod TOCTOU window.
