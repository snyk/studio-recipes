...
def _evaluate_worse_vulns(
    ...
    """Apply worse-vulns criteria per the snyk-sca-fix logic.

    - Newly added package (old is None): flag all vulns
    - Removed package (new is None): skip all vulns (safe)
    - Version change: only flag vulns with severity >= what we'd expect
      to accept (i.e., always flag -- the agent will compare before/after
      when it runs the full SCA fix flow)

    For version changes we flag all vulns found on the changed package,
    because the background scan only sees the *current* state. The
    followup message includes old/new versions so the agent can apply
    the full before-vs-after severity comparison via snyk_sca_scan.
    """
    ...
    if is_code_file(file_path):
        ...
        with _state_lock(workspace):
            ...
            if isinstance(manifests, list):
                manifests = {m: {"changed_packages": {}} for m in manifests}
...
def handle_stop(data: Dict[str, Any], workspace: str) -> None:
    ...
    if isinstance(manifest_files, list):
        manifest_files = {m: {"changed_packages": {}} for m in manifest_files}
