...
def handle_post_tool_use(data: Dict[str, Any], workspace: str) -> None:
    ...
    if is_code_file(file_path):
        ...
        # Build edits list from Claude's tool_input format
        edits = []
        if tool_name == "Edit":
...
def handle_stop(data: Dict[str, Any], workspace: str) -> None:
    ...
    if sca_vulns:
        reason_parts.append("\n## SCA Vulnerabilities in Changed Packages\n")
        reason_parts.append(_format_sca_vuln_table(sca_vulns))

    if sca_fallback_needed and manifest_files:
        all_changed_pkgs = _collect_all_changed_packages(manifest_files)
        if all_changed_pkgs:
            reason_parts.append("\n## Dependency Changes (SCA scan incomplete -- evaluate via MCP)\n")
            for mf, mf_info in manifest_files.items():
                pkg_changes = mf_info.get("changed_packages", {})
                if pkg_changes:
                    reason_parts.append(f"### {Path(mf).name}")
                    reason_parts.append("| Package | Old Version | New Version |")
                    reason_parts.append("|---------|-------------|-------------|")
                    for pkg, versions in pkg_changes.items():
                        old = versions.get("old") or "(added)"
                        new = versions.get("new") or "(removed)"
                        reason_parts.append(f"| {pkg} | {old} | {new} |")
                    reason_parts.append("")
            reason_parts.append(
    ...
    log_to_panel(f"[SAI] Blocking: {len(new_vulns)} code vuln(s), {len(sca_vulns)} SCA vuln(s)")
