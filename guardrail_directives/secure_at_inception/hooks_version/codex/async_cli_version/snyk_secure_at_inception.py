...
WORKFLOW:
  ...
  2. PostToolUse (apply_patch|Edit|Write) -> track modified lines, launch scan
...
def _whole_file_ranges(file_path: str) -> List[Dict[str, int]]:
    ...
    try:
        line_count = max(1, Path(file_path).read_text(
...
def handle_post_tool_use(data: Dict[str, Any], workspace: str) -> None:
    ...
    manifests_tracked: List[str] = []

    if tool_name == "apply_patch":
        patch_text = _extract_patch_text(tool_input)
        if not patch_text:
            debug_log(f"apply_patch with no recoverable envelope; tool_input keys: "
    ...
    if edits_tracked:
        log_to_panel(f"[SAI] Tracked {len(edits_tracked)} code file(s): "
                     f"{', '.join(Path(p).name for p in edits_tracked)}")
    if manifests_tracked:
        log_to_panel(f"[SAI] Manifest tracked: "
                     f"{', '.join(Path(p).name for p in manifests_tracked)}")
