...
from analyze_diff import PackageChange, StagedChanges
                ...
                if by_package:
                    ...
                    for pkg, counts in sorted(by_package.items()):
                        if counts["critical"] > 0 or counts["high"] > 0:
                            transitive_parts.append(
                                f"{pkg} ({counts['critical']}C/{counts['high']}H)"
                            )
...
class FilterResult:
    ...
    def block_reason(self) -> str:
        ...
        if self.package_regressions:
            ...
            reasons.append(
                f"{count} package{'s' if count > 1 else ''} with security regressions: {pkgs}"
            )
...
def filter_new_sast_vulns(
    sast_result: SastScanResult, staged_changes: StagedChanges
    ...
    for vuln in sast_result.vulnerabilities:
        ...
        if file_changes.is_new:
            ...
            new_vulns.append(
                NewCodeVulnerability(vulnerability=vuln, introduced_in_line_range="new file")
            )
        elif file_changes.line_in_changes(vuln.start_line):
            # Vulnerability line is in a modified range
            matching_range = next(
                (r for r in file_changes.modified_ranges if r.contains(vuln.start_line)), None
            )
            range_str = str(matching_range) if matching_range else f"line {vuln.start_line}"

            new_vulns.append(
                NewCodeVulnerability(vulnerability=vuln, introduced_in_line_range=range_str)
            )
        else:
...
def evaluate_package_change(
    package_change: PackageChange, current_scan: ScaScanResult
...
def evaluate_sca_changes(
    sca_result: ScaScanResult, staged_changes: StagedChanges, skip_old_version_scan: bool = False
...
def analyze_staged_changes(
    ...
    if sca_result and sca_result.success and staged_changes.has_package_changes:
        result.package_deltas = evaluate_sca_changes(
            sca_result, staged_changes, skip_old_version_scan=quick_mode
        )
