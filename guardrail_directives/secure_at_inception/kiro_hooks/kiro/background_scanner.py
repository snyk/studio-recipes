...
def handle_after_file_edit(data: Dict[str, Any], workspace: str) -> Dict[str, Any]:
    ...
    if is_code_file(file_path):
        ...
        log_to_panel("[QUEUE] Package manifest changed, SCA scan queued")
...
def handle_scan_complete(data: Dict[str, Any], workspace: str) -> Dict[str, Any]:
    ...
    log_to_panel("=" * 50)
    log_to_panel("⚠️  SECURITY SCAN FOUND ISSUES")
    log_to_panel("=" * 50)
    ...
    return {"exit_code": 0, "user_message": user_message}
    ...
    try:
        ...
        if is_code_file(file_path):
            ...
            if was_pending:
                ...
                tracker.add_packages()
                log_to_panel("[DEBOUNCE] SCA scan delayed")
            else:
                # First change - launch immediately
                log_to_panel("[SCAN] Launching immediate SCA scan")
                launch_background_scan(workspace, "sca", workspace)
                tracker.add_packages()
        else:
            # Not a scannable file
