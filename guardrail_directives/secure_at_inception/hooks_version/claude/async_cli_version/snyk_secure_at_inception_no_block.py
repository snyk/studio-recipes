    ...
    if is_code_file(file_path):
        ...
        if scan_info:
            ...
            if cached_status == "auth_required":
                # No-block variant: notify the user instead of blocking when
                # Snyk is not authenticated.
                log_to_panel("[SAI] Snyk not authenticated — notifying, not blocking")
                clear_scan_state(workspace)  # Allow recovery on next edit
                output_response(
                    {
                        "systemMessage": (
                            "Snyk is not authenticated, so security scanning is "
                            "disabled. Run `snyk auth` in a terminal to enable it."
                        )
                    }
                )
                return
            if cached_status == "snyk_not_found":
...
def handle_stop(data: Dict[str, Any], workspace: str) -> None:
    ...
    sast_failed = False
    sast_auth_required = False
    sca_auth_required = False
    sast_fallback = ""
    sca_fallback = ""
    ...
    if code_files:
        ...
        if scan_succeeded:
            ...
            if scan_status == "auth_required":
                # No-block variant: notify, don't block, when not authenticated.
                log_to_panel(
                    f"[SAI] Snyk CLI not authenticated: {error_detail}"
                    if error_detail
                    else "[SAI] Snyk CLI not authenticated — notifying, not blocking"
                )
                sast_auth_required = True
    ...
    if code_files:
        ...
        if sca_status == "success":
            ...
            if sca_status == "auth_required":
                # No-block variant: notify, don't block, when not authenticated.
                log_to_panel("[SAI] SCA skipped: Snyk not authenticated — notifying, not blocking")
                sca_auth_required = True
            else:
    ...
    # --- No-block variant: notify (don't block) when the only blocker is auth ---
    if (sast_auth_required or sca_auth_required) and not new_vulns and not new_sca_vulns:
        log_to_panel("Stop: NOTIFY (auth) — allowing stop without blocking")
        clear_state(workspace)
        output_response(
            {
                "systemMessage": (
                    "Snyk is not authenticated, so security scanning was skipped. "
                    "Run `snyk auth` in a terminal to enable scanning."
                )
            }
        )
        return
