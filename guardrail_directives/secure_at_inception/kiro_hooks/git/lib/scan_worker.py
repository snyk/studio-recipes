...
from cache import SnykCache, get_cache_dir  # noqa: E402 — imports follow sys.path setup
from run_snyk_scan import run_sast_scan, run_sca_scan  # noqa: E402 — imports follow sys.path setup
...
def safe_open_within_base(file_path: Path, base_dir: Path, mode: str = "r"):
    ...
    try:
        ...
        raise ValueError(
            f"Path traversal detected: {file_path} resolves outside of {resolved_base}"
        ) from None
...
def scan_sast(file_path: str, workspace: str) -> Dict[str, Any]:
    ...
    if cached:
        ...
        return {"status": "cached", "file": file_path, "vuln_count": len(cached.vulnerabilities)}
    ...
    file_vulns = [
        ...
        if Path(v.file_path).name == Path(file_path).name
...
def scan_sca(workspace: str) -> Dict[str, Any]:
    ...
    for _key, pkg_data in packages.items():
...
def main() -> None:
    ...
    parser.add_argument(
        "--type", choices=["sast", "sca"], required=True, help="Type of scan to run"
    )
    parser.add_argument("--target", required=True, help="Target file (SAST) or directory (SCA)")
