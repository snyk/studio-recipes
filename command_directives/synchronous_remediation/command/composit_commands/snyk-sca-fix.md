...
7. **Confirmation for HIGH risk** — In interactive sessions, HIGH risk fixes require explicit user confirmation before manifest changes. MEDIUM risk fixes auto-apply with documented reasoning. In autonomous mode, the agent evaluates the same evidence at HIGH and decides — it must never block waiting for input, and must document its reasoning.
