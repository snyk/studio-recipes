...
class CacheMeta:
    ...
    stats: Dict[str, int] = field(
        default_factory=lambda: {"sast_hits": 0, "sast_misses": 0, "sca_hits": 0, "sca_misses": 0}
    )
...
class SnykCache:
    ...
    def set_sast_result(self, file_path: str, vulnerabilities: List[Dict[str, Any]]) -> None:
        ...
        with FileLock(self.lock_file), open(cache_path, "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cache-dir", default=default_cache, help=f"Cache directory (default: {default_cache})"
    )
