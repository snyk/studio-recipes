...
Cross-platform installer that copies recipes from this repo into **your user home** so **Cursor**, **Claude Code**, **Gemini Code**, **Kiro**, and/or **Windsurf** can use them globally (hooks, commands, skills, MCP merge). Kiro and Windsurf install commands, skills, and MCP only — no SAI hooks.
...
Paths are resolved under `$HOME` (e.g. `~/.cursor/...`, `~/.claude/...`, `~/.gemini/...`, `~/.kiro/...`, `~/.codeium/windsurf/...`, `~/.agents/skills/...` for Windsurf skills). The installer **detects** which ADEs you use or you can target one with `--ade`.

| Recipe type (in manifest) | Typical outcome |
|-----------------------------|-----------------|
| **hooks** | Hook scripts + JSON merge into Cursor `hooks.json`, Claude `settings.json`, or Gemini `settings.json` (Kiro and Windsurf are not configured for hooks) |
| **command** | Slash commands under `.cursor/commands/`, `.claude/commands/`, `.gemini/commands/`, `.kiro/steering/`, or Windsurf global workflows under `.codeium/windsurf/global_workflows/` |
| **skill** | Cursor: skills under `.cursor/skills/snyk/...`; Kiro: skills under `.kiro/skills/...`; Windsurf: skills under `~/.agents/skills/<skill>/SKILL.md`; Claude / Gemini: transformed into a command `.md` |
| **mcp** | Merge of Snyk MCP server entries into `~/.cursor/.mcp.json`, `~/.claude/.mcp.json`, `~/.gemini/settings.json`, `~/.kiro/settings/mcp.json`, or `~/.codeium/windsurf/mcp_config.json` (source: `mcp/.mcp.json` in the repo) |
...
| `--ade <cursor\|claude\|gemini\|kiro\|windsurf>` | Install only for one ADE (otherwise auto-detect / prompt) |
