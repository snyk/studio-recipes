...
Single-file installer for Snyk Studio recipes. It unpacks an embedded payload and installs into your **home directory** so **Cursor**, **Claude Code**, **Gemini Code**, and/or **Windsurf** can use the bundled hooks, slash commands, skills, and MCP configuration. Windsurf installs commands, skills, and MCP only — SAI hooks are not configured for it.
...
Installs into paths under `$HOME` (for example `~/.cursor/`, `~/.claude/`, `~/.gemini/`, `~/.codeium/windsurf/`, and `~/.agents/skills/` for Windsurf skills). The installer can **auto-detect** Cursor / Claude Code / Gemini Code / Windsurf, or you can target one environment with `--ade`.
...
| `--ade <cursor\|claude\|gemini\|windsurf>` | Install only for that ADE (otherwise auto-detect or prompt) |
...
- Confirms **merged configs** still contain the expected Snyk content: Cursor `hooks.json`, Claude `settings.json` hook entries, Gemini `settings.json` hook entries, and MCP server entries in `~/.cursor/.mcp.json`, `~/.claude/.mcp.json`, `~/.gemini/settings.json`, and `~/.codeium/windsurf/mcp_config.json`, as defined in the bundled manifest.
