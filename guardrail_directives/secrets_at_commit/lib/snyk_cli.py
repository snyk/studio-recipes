...
def _pin_problem(pinned: str) -> Optional[str]:
    ...
    expanded = os.path.abspath(os.path.expanduser(pinned))
...
def _snyk_cli_from_sidecar() -> Optional[str]:
    """Returns the installer-pinned CLI path, or None if it cannot be read."""
...
def _augment_path_for_snyk(env: Dict[str, str]) -> None:
    ...
    if pinned:
        ...
        return
    if os.path.lexists(_cli_path_sidecar()):
        return
...
def run_secrets_scan(
    ...
    try:
        if proc.IS_WINDOWS and invocation.needs_shell:
