...
def resolve_log_file(workspace: str) -> str:
    ...
    Unix path: ``~/.snyk-studio/ades/cursor/ws/<workspace-name>/log.txt``.
    Windows path ``C:\\Users\\<user>\\.snyk-studio\\ades\\cursor\\ws\\<name>\\log.txt``.
    """
    name = _safe_workspace_name(workspace)
    return os.path.join(
        os.path.expanduser("~"), ".snyk-studio", "ades", "cursor", "ws", name, "log.txt"
    )
...
def log(message: str, log_file: str, *, debug: bool = False) -> None:
    ...
    entries (debug=True) are written only when CURSOR_HOOK_DEBUG=1. The
    ...
    if not log_file:
        return
    if debug and os.environ.get("CURSOR_HOOK_DEBUG") != "1":
        return
    try:
