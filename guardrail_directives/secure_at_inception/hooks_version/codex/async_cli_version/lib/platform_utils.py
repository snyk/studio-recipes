...
def get_snyk_config_path() -> str:
    ...
    The ``Path.home()`` fallback resolves the invoking user's home directory,
    which is what Snyk CLI credentials always live under -- even when the agent
    itself is scoped to a project-local ``.codex`` directory via ``CODEX_HOME``.
...
def resolve_log_file(workspace: str) -> str:
    ...
    Unix path: ``~/.snyk-studio/ades/codex/ws/<workspace-name>/log.txt``.
    Windows path ``C:\\Users\\<user>\\.snyk-studio\\ades\\codex\\ws\\<name>\\log.txt``.
    """
    name = _safe_workspace_name(workspace)
    return os.path.join(
        os.path.expanduser("~"), ".snyk-studio", "ades", "codex", "ws", name, "log.txt"
    )
...
def log(message: str, log_file: str, *, debug: bool = False) -> None:
    ...
    entries (debug=True) are written only when CODEX_HOOK_DEBUG=1. The
    ...
    if not log_file:
        return
    if debug and os.environ.get("CODEX_HOOK_DEBUG") != "1":
        return
    try:
