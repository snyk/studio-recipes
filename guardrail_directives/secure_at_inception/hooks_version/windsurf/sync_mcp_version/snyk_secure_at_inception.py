...
"""
Windsurf Hook: Snyk Secure At Inception
======================================
...
WORKFLOW:
---------
1. AI edits a code file (.py, .ts, etc.)   -> post_write_code records it as needing SAST
2. AI edits a manifest file (package.json) -> post_write_code records it as needing SCA
3. AI runs snyk_code_scan                  -> pre_mcp_tool_use clears code file state
4. AI runs snyk_sca_scan                   -> pre_mcp_tool_use clears manifest file state
5. Agent stops                             -> post_mcp_tool_use prompts for any remaining unscanned changes
...
INSTALLATION:
-------------
1. Place this script in .windsurf/hooks/ directory
...
-------------------
{
  "hooks": {
    "post_write_code": [
      {
        "command": "python3 .windsurf/hooks/snyk_secure_at_inception.py >> /tmp/windsurf-hook.log 2>&1",
        "show_output": true
      }
    ],
    "pre_mcp_tool_use": [
...
STATE_DIR = os.environ.get("WINDSURF_HOOK_STATE_DIR", "/tmp")
DEBUG = os.environ.get("WINDSURF_HOOK_DEBUG", "1") == "1"
...
def get_state_file_path(workspace: str) -> str:
    ...
    return os.path.join(STATE_DIR, f"windsurf-sai-{workspace_hash}.json")

def log_to_panel(message: str) -> None:
    """Print message to stderr (visible in Windsurf Hooks output panel)."""
...
def handle_pre_mcp_tool_use(data: Dict[str, Any], workspace: str) -> None:
    """
    Handler for pre_mcp_tool_use hook event.
    ...
    """
    tool_name = data.get("tool_info", {}).get("mcp_tool_name", "")
...
def handle_post_mcp_tool_use(data: Dict[str, Any], workspace: str) -> None:
    """
    Handler for post_mcp_tool_use hook event.
